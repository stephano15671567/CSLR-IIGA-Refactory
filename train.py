...
# Unificamos la ruta de guardado con la fecha actual
args.save_dir = os.path.join(args.save_dir, start_date)

# Creamos la carpeta físicamente en Windows para evitar el error del paso 500
if not os.path.exists(args.save_dir):
    os.makedirs(args.save_dir, exist_ok=True)
else:
    print(f'Experiment folder {args.save_dir} already exists..')
    quit(0)

print(f"\nPutting log and checkpoints in: {args.save_dir}")

# Guardamos la configuración de los hiperparámetros en la carpeta correcta
config_file = os.path.join(args.save_dir, 'exp_config.txt')
...
def run_epoch(model, data, is_train=False, device='cuda:0', n_devices=1, epoch=0, args=None, best_err_so_far=1.0, optimizer=None):
    ...
    for step, (x, x_lengths, y, y_lengths, hand_regions, hand_lengths) in enumerate(data):
        ...
        if is_train:
            ...
            if is_train and step > 0 and step % 500 == 0:
                print(f"\n[AUTO-SAVE] Guardando checkpoint de seguridad - Paso {step}")
                step_ckpt = os.path.join(args.save_dir, 'checkpoint_last_step.pt')
                torch.save({
                    'epoch': epoch,
                    'step': step,
                    'model_state_dict': model.state_dict(),
                    'optimizer_state_dict': optimizer.state_dict(),
                    'best_wer': best_err_so_far  # <--- Asegúrate que este nombre sea igual al del argumento de la función
                }, step_ckpt)
...
if(args.checkpoint):
    ...
    if(args.resume):
        ...
        best_err_so_far = checkpoint.get('best_wer', 999.9) # Recupera el récord anterior
...
for epoch in range(start_epoch, num_epochs):
    ...
    train_loss = run_epoch(model, train_dataloader, True, device=device, epoch=epoch, args=args, best_err_so_far=best_err_so_far, optimizer=optimizer)
    ...
    if(epoch % args.valid_steps == 0):
        ...
        with torch.no_grad():
           val_loss, word_err = run_epoch(model, valid_dataloader, is_train=False, device=device, epoch=epoch, args=args, best_err_so_far=best_err_so_far, optimizer=optimizer)
